...
def image_sketch(decode_array_to_img):
    converted_gray_img = cv2.cvtColor(decode_array_to_img, cv2.COLOR_RGB2GRAY)
    sharpening_gray_image=cv2.bitwise_not(converted_gray_img)
    blur_image=cv2.GaussianBlur(sharpening_gray_image,(111,111),0)
    sharpening_blur_image=cv2.bitwise_not(blur_image)
    sketch_img=cv2.divide(converted_gray_img,sharpening_blur_image,scale=256.0)
